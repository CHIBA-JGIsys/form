...
import { submitHubspotFormIvent4 } from '../utils/submitHubspotFormIvent4'

import styles from '@/styles/Form/IventForm2.module.css'
import { TransitionGroup, CSSTransition } from 'react-transition-group'
import { url } from '@/utils/config'
import 'select2'
import $ from 'jquery'

export default function FormComponentsMain4({
  initialStep,
  initialOsdate,
  storedFormData,
}) {
  ...
  const handleSubmit = async (event) => {
    if (validateStep(step)) {
      ...
      await submitHubspotFormIvent4(formData, router)
    }
  }
          ...
          <div>
            ...
            {step === 2 && (
              <>
                ...
                <div className={styles.list}>
                  <div
                    className={`${styles.label__area} ${styles.label_names}`}
                  >
                ...
                <div className={styles.button__area}>
                  <button className={styles.buttonBack}>
                    <a href='https://irodas.com/irodascareerchange/'>←戻る</a>
                  </button>
                  <button className={styles.buttonNext} onClick={handleNext}>
                    OK
                  </button>
            ...
            {step === 5 && (
              <>
                ...
                {renderFormElement(items.find((item) => item.id === 22))}
                <div className={styles.button__area}>
                  <button className={styles.buttonBack} onClick={handleBack}>
                    ←戻る
                  </button>
  ...
  const renderGageArea = () => {
    return (
      <div className={styles.gageArea}>
        <div
          className={`${styles.progressCircle} ${step >= 1 ? styles.active & { step } : ''
            }`}
        >
          1
        </div>
