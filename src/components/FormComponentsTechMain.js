...
import items from '@/data/itemsTech'
import { submitHubspotFormIventTech } from '../utils/submitHubspotFormIventTech'

import styles from '@/styles/Form/IventTechForm.module.css'
import { TransitionGroup, CSSTransition } from 'react-transition-group'
import { url } from '@/utils/config'
import 'select2'
import $ from 'jquery'

export default function FormComponentsTechMain({ initialStep, initialOsdate }) {
  const router = useRouter()
  const [step, setStep] = useState(initialStep)
  const [formData, setFormData] = useState({
    entry_memo: 'irodasSALON',
    year__c: String(initialOsdate),
  })
  ...
  const handleSubmit = async (event) => {
    if (validateStep(step)) {
      ...
      console.log(formData)
      await submitHubspotFormIventTech(formData, router)
                  ...
                  <label className={styles.label__Name}>
                    ...
                    {formData[item.properties] && (
                      <span className={styles.cleartag}>
                        <img src={url('/img/okclear.png')} alt='' />
                      </span>
                    )}
                  </label>
                  <div className={styles.errorMsg}>
