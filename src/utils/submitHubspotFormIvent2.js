...
export async function submitHubspotFormIvent2(formData, router) {
  ...
  return await axios
    ...
    .then(() => {
      // リダイレクト先URLを設定
      const redirectTo = `/member/input/student/meet?year__c=${formData.year__c}&liberalartsscience__c=${formData.liberalartsscience__c}` // ここにHubSpotでコピーしたリダイレクトURLを使用しても構いません
